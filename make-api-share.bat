@echo off
rem PromptCut - API config share tool (GUI).
...
set "ROOT=%~dp0"
set "CRATE=%ROOT%tools\api-share-gui"
set "EXE=%CRATE%\target\release\api-share-gui.exe"

if exist "%EXE%" goto run

echo Building the share tool (first run only, takes a minute or two)...
