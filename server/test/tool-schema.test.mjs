...
// 三个都要当场起一个 Chrome 渲一帧(vite-plugin-vision):see_preview 和 bake_card 渲单帧,
// see_sequences 渲一整张拼图。
test('放宽超时的只有这三个渲染类工具；都要大于自己那条管线的上限，否则报错信息会被桥的超时盖掉', () => {
  const withTimeout = mcpTools.filter((t) => t.timeoutMs);
  assert.deepEqual(withTimeout.map((t) => t.name), ['see_preview', 'bake_card', 'see_sequences']);
