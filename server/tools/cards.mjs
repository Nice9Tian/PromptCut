export const cardsTools = [
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "detail": {
          ...
          "description": "full 表示所有卡都要完整 schema，通常不需要"
        }
      }
    },
    "side": "browser"
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "file": {
          "type": "string",
          "description": "要读的文件相对路径，必须在这张卡的 files 列表里；不传就是卡片定义文件"
        }
      },
      "required": [
        "cardId"
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "file": {
          "type": "string",
          "description": "要改的文件相对路径，必须在 get_card_source 返回的 files 里；不传就改卡片定义文件"
        },
        "find": {
          "type": "string",
          "description": "要被替换掉的原文，逐字照抄源码"
        },
        "replace": {
          "type": "string",
          "description": "替换成的新内容"
        ...
        "replaceAll": {
          "type": "boolean",
          "description": "find 有意匹配多处且都要改时传 true"
        }
      },
      "required": [
        "cardId",
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        "clipId": {
          "type": "string",
          "description": "要看的片段 id"
        },
        "t": {
          "type": "number",
          "description": "时间轴第几秒；不传取这个片段的中点"
        },
        "ref": {
          "type": "number",
          "description": "从这个节点往下展开（上一次返回里的 ref 数字）；不传从最外层开始"
        },
        "depth": {
          "type": "number",
          "description": "往下展开几层，默认 3，最多 8"
        }
      },
      "required": [
        "clipId"
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        "id": {
          "type": "string",
          "description": "小写 kebab-case，全局唯一，例如 price-tag"
        },
        "source": {
          "type": "string",
          "description": "完整 TSX CardDef 源码"
        },
        "overwrite": {
          "type": "boolean",
          "description": "只在确实要把同名卡整篇换掉时传 true；改细节请用 edit_card"
        }
      },
      "required": [
        "id",
  ...
  {
    "name": "bake_card",
    "description": "把一张卡**烘成一张图片**存进素材库,返回它的 URL。目前唯一的用处是给 `scene-3d` 当贴图 —— 把 URL 填进那张卡的 `texture` 参数,就得到「立体物件表面印着这张卡」。\n\n画这张图的是**导出成片的那个渲染器**(和 see_frames 同一条管线),所以贴上去之后预览和成片长得一样。\n\n**它是一张快照,不是活的**:卡片的动画会定格在 `t` 那一帧;之后你改了这张卡的参数,贴图**不会**跟着变,要重新烘一次再把新 URL 填回去。所以顺序是「先把卡调好,再烘」。\n\n两种观感,由 `bg` 决定,**烘的时候就定死**:不传 bg = 透明底,物体在卡片没画的地方也透空,内容像浮在空间里(适合标志、招牌);传了 bg = 压平成不透明,得到实心物体表面印着这张卡 —— 大多数时候说「把卡贴到立方体上」要的是这个。\n\n画幅会被改成正方形再渲(贴到立体表面上,16:9 会被拉变形);卡片本来就是响应式的,所以这是重排不是裁切。素材段(视频 / 图片)不用烘,它本来就是位图,直接拿它的 URL 当贴图即可。",
    "inputSchema": {
      "type": "object",
      "properties": {
        "clipId": {
          "type": "string",
          "description": "要烘哪一张卡(时间轴上的片段 id)"
        },
        "t": {
          "type": "number",
          "description": "烘哪一刻的样子(秒,时间轴绝对时间)。不传取这一段的中点 —— 起止两端常卡在进出场动画上,烘出来是个半透明的中间态"
        },
        "size": {
          "type": "number",
          "description": "贴图边长(像素,正方形),256~2048,默认 1024。要贴的物件在画面里很小就调小,省内存"
        },
        "bg": {
          "type": "string",
          "description": "底色,六位十六进制如 \"#0b0f17\"。不传 = 透明底(挖空观感);传了 = 实心观感"
        }
      },
      "required": [
        "clipId"
