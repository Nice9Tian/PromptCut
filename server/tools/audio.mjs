    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "scope": {
          ...
          "description": "不传就按给了 clipId 还是 mediaId 判"
        },
        "series": {
          "type": "boolean",
          "description": "clip / media 档也要逐秒曲线时传 true(timeline 档总是给)"
        }
      }
    },
    "side": "browser"
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        "mediaId": {
          "type": "string",
          "description": "素材库里的视频:派生一份声音素材"
        },
        "clipId": {
          "type": "string",
          "description": "时间轴上的一段视频:就地转成声音"
        }
      }
    },
    "side": "browser"
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        "name": {
          "type": "string",
          "description": "素材库里显示的名字,30 字以内"
        },
        "description": {
          "type": "string",
          "description": "一句话说它是什么效果、适合什么声音"
        },
        "params": {
          "type": "object",
        ...
        "clipId": {
          "type": "string",
          "description": "可选:建完直接挂到这一段(视频 / 声音)"
        },
        "clipParams": {
          "type": "object",
          "description": "可选:挂上时这一段的参数值,覆盖 params 的 default"
        }
      },
      "required": [
        "name",
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "fxId": {
          "type": "string",
          "description": "空字符串 = 摘掉这一段的音频效果"
        },
        "params": {
          "type": "object",
          "description": "可选:这一段的参数值,只能是这个效果 params 里声明过的"
        }
      },
      "required": [
        "clipId",
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        "text": {
          "type": "string",
          "description": "要念的文字。数字、英文按想要的读法写(「130 毫秒」「PromptCut 零点五」)"
        },
        "start": {
          "type": "number",
          "description": "放到时间轴的起点(秒);不传只进素材库"
        },
        "trackId": {
          "type": "string",
          "description": "放到哪条序列;不传自动挑"
        },
        "provider": {
          "type": "string",
          "enum": [
          ...
          "description": "不传用设置里的默认服务"
        },
        "voiceId": {
          "type": "string",
          "description": "音色 id,只能取 voice_list 里有的;不传用该服务的默认音色"
        },
        "speed": {
          "type": "number",
          "description": "语速,MiniMax / Vidu 0.5~2、可灵 0.8~2;不传用设置"
        },
        "emotion": {
          "type": "string",
          "enum": [
          ...
          "description": "情绪,只对 MiniMax / Vidu 有效;不传由模型按文字判断"
        },
        "name": {
          "type": "string",
          "description": "素材名里带上的简短标签,如「开场旁白」"
        }
      },
      "required": [
        "text"
