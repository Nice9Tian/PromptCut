export const layoutTools = [
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "space": {
          ...
          "description": "默认 local;卡片级两者等价"
        },
        "x": {
          "type": "number",
          "description": "锚点的横坐标(像素)"
        },
        "y": {
          "type": "number",
          "description": "锚点的纵坐标(像素)"
        },
        "w": {
          "type": "number",
          "description": "画布宽(像素),省略=舞台宽"
        },
        "h": {
          "type": "number",
          "description": "画布高(像素),省略=舞台高"
        },
        "anchor": {
          "type": "array",
          "items": {
          ...
          "minItems": 2,
          "maxItems": 2,
          "description": "[ax, ay],0~1;默认 [0,0]"
        },
        "scale": {
          "type": "number",
          "description": "绕锚点缩放,默认 1"
        },
        "rotate": {
          "type": "number",
          "description": "绕锚点旋转,度,顺时针,默认 0。这是**平面内**的旋转"
        },
        "rotateX": {
          "type": "number",
          "description": "三维:绕水平轴翻转,度。正值=**顶边往里倒、底边朝观众抬起来**(像把牌子朝后仰)。**要先用 set_camera3d 打开三维**,否则只会看到仿射拉伸而不是透视。**只对卡片生效**,素材段(视频/图片)会被拒"
        },
        "rotateY": {
          "type": "number",
          "description": "三维:绕垂直轴翻转,度。正值=**右边往里转、左边朝观众转过来**。同样要先 set_camera3d,同样只对卡片生效"
        },
        "translateZ": {
          "type": "number",
          "description": "三维:沿深度平移,舞台像素,正值朝观众(变大)、负值往里(变小)。同样要先 set_camera3d,同样只对卡片生效。注意推得太靠前会越过相机:translateZ 接近\"相机距离\"(set_camera3d 会告诉你这个数)时卡片会涨到占满整幅甚至更大"
        },
        "clear": {
          "type": "boolean",
          "description": "true = 删掉框,恢复铺满全屏"
        },
        "clamp": {
          "type": "boolean",
          "description": "true = 算完后把可见框夹回舞台内,不让卡片出画"
        }
      },
      "required": [
        "clipId"
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "align": {
          ...
          "minItems": 2,
          "maxItems": 2,
          "description": "在矩形里靠哪:[0,0] 左上、[0.5,0.5] 中心(默认)、[1,1] 右下"
        }
      },
      "required": [
        "clipId",
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "clamp": {
          "type": "boolean",
          "description": "true = 算完后把可见框夹回舞台内,不让卡片出画;微调时建议带上"
        }
      },
      "required": [
        "clipId"
  ...
  {
    "name": "set_camera3d",
    "description": "打开 / 关掉三维透视,并调它的强度。**这是三维的唯一开关**,整个项目一档,不是逐卡的。关着的时候 set_position 的 rotateX / rotateY / translateZ 只会得到仿射拉伸 —— 卡片被斜切,但没有近大远小,看着像贴纸不像立在空间里。打开之后舞台变成一个透视空间:屏幕平面是 z=0,translateZ 正值朝观众来(变大)、负值往里去(变小)。\n\n强度用 fovDeg(视场角)调,**没有\"相机距离\"这个参数** —— 距离是从 fov 和画布高度推出来的(d = H / (2·tan(fov/2)))。这么设计是因为竖屏项目画布高 1920、横屏 1080,同一个距离在两种画幅下透视强度完全不同,而 fov 直接就是\"透视有多强\",换画幅不用重调。档位:**30° 克制**(接近正交,适合规整的信息版面)、**40° 默认**、**50~60° 明显**(卡片一转就有纵深)、**80° 以上是鱼眼**,边角会夸张变形。\n\n改完一定要 see_frames 看真实画面:透视强度只能看出来,算不出来。",
    "inputSchema": {
      "type": "object",
      "properties": {
        "enabled": {
