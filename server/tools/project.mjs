  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        ...
        "duration": {
          "type": "number",
          "description": "整条片子多长,单位秒。时间轴从 0 开始、到这里结束,**预览和导出都在这一刻切断**。它不会自己跟着内容走:加卡片不会把它撑长,删东西也不会把它缩短(只有拖素材上轨道时会往长了顶一次)。所以排完版要自己对一遍 —— 工具返回的 timeline 里 contentEnd 是内容实际结束的位置,比 duration 小就是片尾挂了一段黑,比 duration 大就是后面那截被切掉了,两种都要把 duration 设成 contentEnd。"
        },
        "themeId": {
          "type": "string",
          "description": "全局主题 id"
  ...
  },
  {
    ...
    "inputSchema": {
      ...
      "properties": {
        "url": {
          "type": "string",
          "description": "附件的站内地址，取自用户消息末尾的附件清单"
        },
        "name": {
          "type": "string",
          "description": "文件名，不传就从地址里取"
        }
      },
      "required": [
        "url"
