...
- **项目里有视频素材,排卡之前必须先 `see_sequences` 看过画面**(详见工具 §12)。
...
- **收工前把整条片子按时间通检一遍,不要只盯着你最后碰的那张卡。**
  一次真实的失败(20 秒、10 张卡):模型调了 20 次 `see_preview`,其中**整屏只看过 `t=0`**,
  ...
  用不带 `clipId` 的 `see_preview({ t })` 看整屏(要看的是"这一刻观众看到什么",
...
- **同一个调用连着给出同样的结果,就不要再调第三次了。**
  上面那次失败里,模型把同样的 5 次 `see_preview` **原样重复了三轮**,每轮拿到的都是同一张图。
  重复调用不会带来新信息 —— 该做的是换个手段(看整屏而不是单卡、换个 `t`、`get_clip` 看封装、
  `get_layout` 看框),或者直接把"我卡在哪、看到的是什么"写给用户,让人来判断。

- **一片棋盘格 = 那一刻真的什么都没有。** `see_preview` 的灰色棋盘格是透明,不是内容。
