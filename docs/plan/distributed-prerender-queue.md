...
- **谁定指纹（卡片级指纹锁，2026-09-24 改为按卡锁定）**：锁的单位是「一张卡的一种结果」，锁键 `<kind>:<input.contentKey>`（快照、轨道流各一把）。最先产出这种结果的环境锁定它：页面测量推过的帧入库时用页面的指纹锁定；队列里第一次认领时用任务的指纹锁定。
  - 认领 `plan` 任务的节点切分时，没被锁的卡按**自己的**指纹出键；
  - 已被别的环境锁定的卡，照锁定方的指纹出键、写进 `requires.envFingerprint`，剩余帧只有同指纹的节点能认领（第 4.3 节规则 1）；
  - 要接手，就用自己的指纹另起一套键，发布时带 `takeover`，锁转给它，原指纹的未完成任务作废（`superseded`）；
  - 锁由文档服务的队列记，认领时再查一次（`card-locked`），见 `render-queue-contract.md` F 节。
- **效果**：
  - 每一层（一张卡的一种结果）的所有帧都出自同一种环境，页面贴的连续帧不会混环境；同一版项目里不同的卡可以出自不同环境；
...
| `source.tenantId` | 文档服务 | 发布方凭证所属的共享项目（主执行计划 D9，第 12 节），同样按凭证填写；只用于本机 PC、独立渲染主机的授权范围，**不放宽**纯浏览器节点的边界 |
...
- 纯浏览器节点只能看到、只能认领**当前登录用户自己产生的**任务（`source.userId` 等于节点凭证的用户）。同一共享项目里其他成员的任务也不行。
...
- **Q2 纯浏览器节点的边界**：只处理**当前登录用户自己产生的**任务，禁止任何跨用户调度，同一共享项目的其他成员也不行。由文档服务按凭证把关，不靠节点自己过滤（第 4.3 节规则 0、第 4.2 节 `forbidden`）。理由：杜绝端侧算力被不可控地占用，杜绝隐私越权。
