...
- **先验再牵线**：成员向云端托管服务要主机地址之前，先交项目凭证的证明。不对就直接拒绝：不给地址、不打洞、不中继。拒绝同样计入第 9 节的限速。
- **怎么核对**：同第 4、5 节的挑战应答。云端托管服务发一次性随机数和盐，名单外的用户名给伪盐（第 4 节）；成员算证明交回，按定长比较；随机数只用一次、核对后作废。
- **存什么**：凭证的校验派生值，按第 3 节项目记录的形状：`kdf`；自由进入的 `project: { salt, key }`；限定进入的 `list: [{ username, salt, key }]`。不存口令本身，不存 `ticketKey`、代数。
- **谁同步、什么时候**：主机在向云端托管服务登记时同步一次；`set-password`、`set-list`（第 7 节）成功后再同步。主机离线时跳过，恢复后补；补上之前，云端托管服务以旧值为准。
- **主机再验一次**：成员连上主机后（公网直连或经中继），仍按第 5 节向主机的文档服务交证明，主机核对通过才进入。禁入表（`kick`）只在主机上核对。
- **票据**：仍由主机的文档服务按第 8 节签发。云端托管服务不持有 `ticketKey`，不签票据。

**待定**（写法与用户确认后补）：
1. 同步的是第 3 节的 `K` 本身，还是由 `K` 再派生出的专用值（例如 `HMAC-SHA256(K, "promptcut.rendezvous.v1")`，成员也用它算给云端托管服务的证明）。本协议里 `K` 等同于口令（主执行计划第 12.2 节「口令怎么核对」一行）：云端托管服务拿到 `K`，就能以任何成员的身份通过主机的核对，第 16 节「主机再验一次」对它不起作用；换成专用值则没有这个问题。
2. 交给云端托管服务的证明，用途串是否另起前缀（如 `"promptcut.rendezvous.v1\n"`），使它不能拿到主机那边重放。
3. 创建者那一份（`creator`）要不要同步：创建者在本机进入自己的项目，不经牵线，建议不同步。
4. 禁入表要不要同步给云端托管服务，让被踢的设备在牵线这一步就被拒。
