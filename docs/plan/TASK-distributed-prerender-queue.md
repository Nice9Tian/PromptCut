...
| **M4 环境指纹进结果键**（2026-09-24 完成；实际范围见 `render-queue-contract.md` E 节：另含独立卡 PNG 缓存键、测量帧入库的环境闸） | 设计 2.1；`TODO.md`「语义与代码的差距」那一条 | 本机的共享键、本地档键、流键乘上指纹；缓存整体换键一次 | M2 的指纹函数 | 基线全绿；`verify-determinism`、`verify-unified-frames` 通过；`ready-index-probe`、`stream-produce-probe` 通过；导出像素基线不变（指纹只影响预渲染键，不影响导出） |
