...
| **M3 进程内联调**（2026-09-24 完成；实际范围见 `render-queue-contract.md` D 节：不改 `frame-pipeline.mjs`，接真实执行器挪到 M5） | M1、M2 | 本机预渲染进程作为节点接到**进程内**的队列上（假传输 + 假产物去重接口），`preload` 仍是默认路径，队列路径在开关后面 | M1、M2 | 两个进程内节点抢同一批任务，每个任务恰好完成一次；中途「断开」一个节点，宽限期后另一个接手；现有探针（`ready-index-probe`、`preview-fallback-probe`）照旧通过 |
| **M4 环境指纹进结果键**（2026-09-24 完成；实际范围见 `render-queue-contract.md` E 节：另含独立卡 PNG 缓存键；测量帧入库改为卡片级指纹锁，见 F 节） | 设计 2.1；`TODO.md`「语义与代码的差距」那一条 | 本机的共享键、本地档键、流键乘上指纹；缓存整体换键一次 | M2 的指纹函数 | 基线全绿；`verify-determinism`、`verify-unified-frames` 通过；`ready-index-probe`、`stream-produce-probe` 通过；导出像素基线不变（指纹只影响预渲染键，不影响导出） |
