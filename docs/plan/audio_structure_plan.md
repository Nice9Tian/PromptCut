...
- 前一轮：文档服务只存工程文档，素材服务只管字节，服务端一律不处理音频。
...
- **范围**：预览播放、音频图卡求值、导出混音、响度测量、时间轴波形，一共五块，全部改到浏览器里做。**不在范围内**：
  ...
  - 画面的逐字节对账。
- **桌面运行环境导出时，ffmpeg 只做一件事**：把页面编好的 AAC 按 `-c copy` 原样装进 mp4，不解码、不混、不重编码。这一步只是装进容器，不算音频处理。要不要连这一步也搬进 JS，是第 6 节待定 2。
- **「浏览器端」的含义**：
  - 桌面运行环境下，是编辑器页面加上 puppeteer 开的导出 / 混音页；
  - 在线浏览器模式下，是用户自己的标签页。
...
- **读取**：桌面运行环境对 `/@media/<文件>` 发 HTTP Range 请求（静态文件服务，不算处理）；在线浏览器模式对所连素材服务的地址发同样的请求；拖进来的文件用 `File.slice`。只读索引和用到的那几块，不整份读。
...
- 按那份计划第 2 节实现：
  ...
  - 就绪索引里加一类 `'audio'`。
- **PC 客户端上传、浏览器拉取**：接口形状照 `cloud-task.md` A3b 的预渲染产物入库（推送到素材服务），第一版可以只把接口留好，和「在线重型控件服务」一样先留口子。
...
5. **这几条要不要写进 `docs/semantics/`**（pinned 已归档，被它取代）：「音频全部浏览器端 JS、流式分段」「音频比对按 PCM 最大差 8%」。要的话，我拟好原文弹窗给你勾选。
