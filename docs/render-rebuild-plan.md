...
| MCP 衔接 | 完成 | `get_card_source` / `edit_card` 覆盖内置卡和卡片用到的部件文件(带共用计数、备份、校验);新增只读的 `inspect_card_dom`(每个节点标源码行,经 source map 换算)。端到端实测:无头编辑台连桥 → `/api/mcp/call` 走完三个工具;60 KB props 的项目照常出树 |
