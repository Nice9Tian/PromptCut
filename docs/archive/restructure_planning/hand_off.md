...
交接时间：2026-09-22，R7b 合并之后。写给下一个接手的人（人或 Agent）。读这一份就能知道：东西在哪、做到哪、哪些是自报没复核的、哪些明确没做、下一步先做什么、谁要拍板。
...
**复核查出的 5 处不符合 pinned 目标和 1 条回归，已由 R7b 修掉并合并**（`npm test` 1648 / 1647 / 0 失败；每条各有探针断言，Agent 自报；详见 check 的 R7b 节）。下表留作记录：
...
回归（播放头在末尾按播放不重播，R7-11）同样已修。**R7b 没做成的三条**：`stageSwap` / `snapshotFeed` / `demote` / 节拍循环没有单测（import 无扩展名，`node --test` 不认）；渲染 9 只停了快照没停 PNG（legacy 通道还要用）；架构 10 停顿进行中音频照播（要不要加看门狗等用户定）。
...
5. 先修第 2 节那 5 条 + 播到头回归，再开 R8——**R7b 已做完合并**，报告 `reports/r7b-report.md`。

**还等拍板**：
5b. 架构 10：停顿进行中要不要加播放中的 40 ms 看门狗定时器让音频立刻停（现在只在下一条 frame 到达时事后判）。
...
2. ~~顺手清 R5～R7 的零碎~~（R7b 做了 8 条）；剩：给 `stageSwap` / `snapshotFeed` / `demote` / 节拍循环补单测（先给它们的 import 补 `.ts` 后缀）；把 `reports/r7b-report.md` 第 4 节的 8 条更正折回 `r2-r7-task.md`。
