...
## 追加：按契约第 9 节返工（2026-09-25）

主会话集成后，只有 `artifact-push` 的 W4 失败（「四段都落进了 push-queue.json」），裁定写进了契约第 9 节。本分支改了两处：

1. **`enqueue` 返回落盘的 promise**（dd0adec，第 9 节第 1 条）：
   - 队列文件的写回改成一条链。排在链尾、还没开始的那次写入，在开始时才取快照，所以它开始前的改动都由它一并写下；已经开始的写入不再收新改动，之后的另排一次；
   - 因此连续多次 `enqueue`，每个返回的 promise 都在包含了它那一段的写入完成后才兑现；
   - 进队本身仍然同步生效。重复的段只合并优先级，也回写入的 promise。不合格的段回一个已兑现的 promise；
   - 管线的两个钩子只 `void` 它，不 `await`，不挡预渲染。
2. **`stop()` 不再等在推的段**（27ee356）：
   - 修完第 1 处后，测试方的 W4 会卡死：它在某一段推送被卡住时 `await stop()`，要等 `stop()` 返回之后才放开那一段。而原来的 `stop()` 要等在推的段收尾，两边互相等；
   - 现在 `stop()` 只停派新活，等队列文件写完就返回；
   - 在推的段之后自己收尾：成功就出队，失败就留在文件里，都照常写回。

第 9 节第 2 条（`priority` 是最终级别，队列不再二次判定）与本实现不完全一致，这一处没改：

- 本实现的队列仍然取「调用方的级别」「这一段自己看得出的下限（流、本地档、`canvasHeavy`）」「块级（`data:image`、超体积）」三者中最低的一级；
- 钩子给的卡级加上队列算的块级，结果与第 4 节的表相同。调用方直接给 2 时，队列不会把它降回 0 或 1，所以取最低一级只会压低、不会抬高；
- 但调用方直接 `enqueue` 一段本地档或含超体积帧的段、又给 `normal` 时，队列会把它压成 `low` / `lowest`，不是原样照用；
- 测试方的 W2 / W3 在这个读法下全过。要不要改成完全照用调用方的级别，请主会话定。

第 10 条（编辑器里挂的文档服务的发现）按裁定并入 M5b，本分支不做。
...
| `npm test` | 退出码 0：tests 2352，pass 2351，fail 0，skipped 1（需要 5190 的那一条） |
| 测试方的 C6.4 四个测试文件 | 见下，全过 |
| 自测 | 10/10。新增一条：连续 4 次 `enqueue`，逐个 `await` 后读文件，每次都至少有已进队的段数；重复段只合并优先级；不合格的段回已兑现的 promise |

测试方文件的跑法：

1. 临时建了一个 detached worktree `.worktrees/c6-4-pipeline-check`，基于本分支；
2. 从 `claude/c6-4-tests`（fd0f4b7）取 `server/test/`，从 `claude/c6-4-node` 取 `server/render-node/`，不提交；
3. 逐个文件跑，结果：`content-client` 7/7、`artifact-dedup` 7/7、`artifact-push` 7/7（含 W4）、`artifact-adopt` 3/3；
4. 跑完查过没有 junction（0 个），已删除这个 worktree。

自测原来的 W4 式用例是在 `q1` 还有在推的段时就重建队列。按新的 `stop()` 语义，这种写法下同一段会被两个队列各推一次：块照样只上传一次，但清单写了两次。所以改成先等 `q1` 在推的段收尾、再重建，模拟进程真的退出之后才重启。
