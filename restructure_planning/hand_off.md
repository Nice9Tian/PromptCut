...
## 4. 等用户拍板的（复核列出的 5 条 + 计划第 7 节的 3 条）

1. `Date.now` 固定成 2026-01-01，还是按计划取舞台打开时的真实时间（R3-17）。
2. 导出页 / 预渲染页不装虚拟定时器，能否接受（R3-18）。
3. pinned 渲染 2「抽 8 帧取最差」与渲染 5「p90 + 最少采样帧数」字面冲突，代码按渲染 5 做，是否认可；认可的话渲染 2 要经弹窗改字。
4. pinned 渲染 4 被渲染 3 的「整段最差代价」取代，是否认可；同上要改字。
5. **是否先修第 2 节那 5 条 + 播到头回归**，再开 R8。我的建议：先修，这几条都是用户能直接感到的，而且都在 R5～R7 的范围内，不算新工作。
6. R8、R9、云端三份分册动工前要不要派独立审查、派谁（R2～R7 那份没派，复核已经替代了一部分）。
7. `cloud-task.md` 文末 6 个问题（云端动工前）。
8. R8 / R9 分册的三个细节（各自动工时）。

另：`reply_to_users_goal.md` 第 9 行还是旧的上传顺序；fold-notes 要求把「原片 = 原编码」的 `playable` 读法写进去请用户确认，没做（DEC 另）。

## 5. 下一步建议（按顺序）

1. **修第 2 节的 5 条 + 回归**（约一个 R 步的量）：
