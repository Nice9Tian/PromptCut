...
1. R8、R9、云端三份分册都还没经过独立审查（R2～R7 那份也没派，实现者对照代码逐句读过后已完成）：各自动工前要不要派、派谁。
...
- 轨道流的编码原型现在就做、不等 R7：用户明确要面向低配机、60 fps 下保证流畅。原型（原 G0-b：解码吞吐、编码耗时与 alpha 误差、严格 GOP 参数、fMP4 切分、裁剪矩形取法）只产出探针、数据和报告，不碰主线；R8 的实现仍排在 R7 之后。
- 代码注释和文档里的禁用旧词做一次全仓清理（只动注释和文档，字符串字面量只列不改），单独合并、不混进 R2～R7。
