...
export function CodeTab({ clip }: { clip: TrackClip }) {
  ...
  const handleReset = () => {
    actions.setClipParams(clip.id, {}, { merge: false });
    const nextCurrent = JSON.stringify({ cardId: clip.cardId, start: clip.start, end: clip.end, params: {} }, null, 2);
    ...
    <div className="flex flex-col h-full p-2">
      ...
      <div data-pc="code-status" className="mt-2 shrink-0 h-4">
        {errorMsg ? (
          ...
          <div className="text-[10px] text-neutral-500">可改:cardId / start / end / params</div>
        )}
      </div>
    </div>
  );
