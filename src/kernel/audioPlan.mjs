 ...
 *
 * 三处用同一份规则,免得各写一遍又对不上:
 *   - 导出混音(server/bakery/mux-audio.mjs 的 buildAudioPlan → ffmpeg;server/bakery/audio-mix.mjs 的 Chrome 离线渲染)
 ...
 * 和 ffmpeg 那条路(server/bakery/mux-audio.mjs:afade t=in 再 afade t=out,默认 tri 曲线)语义完全一样 —— 两条淡化相乘。
