...
const signature = (project: Project) => JSON.stringify([project.width, project.height, project.fps, project.duration, project.themeId, project.camera3dFov, project.tracks, project.media, project.filters, project.audioFx]);
...
export function restoreSnapshots(project: Project, snapshots?: string) {
  ...
  restorePending = saved
    ? frameRequest("import", project, { snapshots: saved.snapshots }).then(() => undefined, () => undefined)
    : null;
}
export async function frameRequest(operation: string, project: Project, extra: Record<string, unknown> = {}, signal?: AbortSignal) {
  const url = await prerenderUrl(`/api/frames/${operation}`);
  const res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project, ...extra }), signal });
...
export async function collectSnapshots(project: Project) {
  const result = await frameRequest("archive", project);
  saved = { signature: signature(project), snapshots: result.snapshots };
  restorePending = null;
}
/** Timeline preview and Agent use this same server queue. */
export async function see_frames(project: Project, times: number[], signal?: AbortSignal) {
  ...
  return frameRequest("see", project, { times }, signal);
}
