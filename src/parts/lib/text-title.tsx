...
function TitlePart({ params, width }: PartProps<Params>) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: align, gap: 8, padding: "0 24px", maxWidth: width }}>
      {params.kicker && (
        <motion.div
          style={{ fontSize: Math.max(14, params.size * 0.55), letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", opacity: 0.85, fontWeight: 300, textAlign }}
      ...
      <motion.div
        style={{ fontSize: params.size, fontWeight: 700, lineHeight: 1.15, color: accentOf(params), textAlign, textShadow: "0 2px 12px rgba(0,0,0,0.35)" }}
        initial={{ opacity: 0, y: 10 }}
...
export const textTitle: PartDef<Params> = {
  ...
  role: "text",
  from: "pin-board",
  defaults: { kicker: "SUMMARY", text: "核心要点总结", size: 48, accent: "", align: "left" },
  controls: [
    { key: "kicker", label: "小字眼", type: "text", hint: "留空不显示" },
    { key: "text", label: "标题", type: "text", required: true },
    { key: "size", label: "字号", type: "number", min: 16, max: 160, step: 2 },
