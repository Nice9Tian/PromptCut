...
function PinsPart({ params }: PartProps<Params>) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", gap: 16, alignItems: "flex-start", justifyContent: "center", padding: "0 16px" }}>
      {items.map((item, i) => (
        <motion.div
          key={i}
          style={{ background: "#111", color: "#fff", padding: "12px 28px", borderRadius: 14, fontSize: params.size, fontWeight: 500, boxShadow: "0 12px 30px rgba(0,0,0,0.45)", border: "1px solid rgba(255,255,255,0.1)" }}
...
export const listPins: PartDef<Params> = {
  ...
  role: "list",
  from: "pin-board",
  defaults: { items: "第一点核心优势|第二点不可替代|第三点极速交付", stepMs: 200, size: 36 },
  controls: [
    { key: "items", label: "要点(用|分隔)", type: "text", required: true },
    { key: "stepMs", label: "间隔(ms)", type: "number", min: 0, max: 2000, step: 20 },
    { key: "size", label: "字号", type: "number", min: 14, max: 96, step: 2 },
  ],
