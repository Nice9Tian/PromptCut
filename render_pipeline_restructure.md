...
3. 轨道流原型（`docs/g0-b-stream-prototype.md`）建议 **`streams` 第一版默认关**：1080p 全幅卡页面侧每帧出图 30.5 ms、只剩 9% 余量，两个编码器并存时 68.9 ms、追不上实时；要默认开得先收紧裁剪矩形、`stride ≥ 2`、隔开编码器和预渲染 worker 的 CPU 竞争。默认开还是关，R8 动工时定。原型另有三条已经写进 R8 分册的硬结论：`out_range` 必须是 `tv`（`pc` 在 Chrome 上不生效）、预乘色要配着色器钳位、单个解码器同时持有 ≤ 8 帧。
4. R8 / R9 分册留的三个细节（稀疏分段的步长是否只能取 15 的因数、G4 里生产侧 `frameMs` 要不要换名、M2 一处长句的读法）——各自动工时定。
